...
// jika tahun kosong gunakan tahun sekarang
$tahunGrafik = !empty($tahun) ? $tahun : date('Y');
...
$queryGrafik = mysqli_query($conn, "
    ...
    FROM pembayaran
    WHERE YEAR(tanggal_bayar) = '$tahunGrafik'
...
while($row = mysqli_fetch_assoc($queryGrafik)){
    ...
    $dataPendapatan[$bulanIndex] = (int)$row['pendapatan'];
}
...
new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            label: 'Pendapatan Tahun <?= $tahunGrafik ?>',
    ...
    options: {
        ...
        plugins: {
            title: {
                display: true,
                text: 'Grafik Pendapatan PDAM Tahun <?= $tahunGrafik ?>'
            }
        },
        scales: {
            y: {
